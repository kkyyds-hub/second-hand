...
**一次性与幂等（必须实现双保险）：**
- 应用层：创建前先查询是否已存在评价（order_id + role 维度）
- 数据库层：唯一约束兜底（见 6.2）
- 若并发导致唯一键冲突：统一转换为业务异常（**“该订单已评价”**，见 7）

> 备注：为了回归断言稳定，“已评价”必须走固定 msg，不允许透出 DB 原始 Duplicate entry 文案。

### 2.2 匿名展示规则（必须冻结，否则前端会打架）
当 `isAnonymous=true`：
- `buyerDisplayName`：返回固定值 **"匿名用户"**
- `buyerAvatar`：返回 **空字符串**（前端使用默认头像）

当 `isAnonymous=false`：
- `buyerDisplayName`：正常返回用户昵称/用户名
- `buyerAvatar`：正常返回头像

> 注意：匿名只影响“展示字段”，数据库仍保存 buyer_id 便于审计与风控（匿名不等于不可追踪）。
...
- 商品评价列表：**必须登录**（因为接口位于 `/user/**` 且拦截器当前会拦截）

> 若未来要“未登录可看评价”，需将该接口迁移到非 `/user/**` 路径或在拦截器显式放行；并必须同步更新本文档版本号后再改实现。
...
> 说明：该约束名 **必须固定为 `uniq_order_role`**，用于将 DB 冲突稳定映射为业务提示语 **“该订单已评价”**（见第 7 节）。
...
## 7. 错误码与提示语冻结（便于回归断言）
> 项目当前多用 BusinessException + msg；Day12 也按该方式冻结“提示语口径”。
...
3. 非本人订单创建评价失败
4. 同一订单重复评价失败（并发/重复提交都能稳定失败，提示语命中“该订单已评价”）
...
- 固定 buyer：`buyer01`
- 固定 seller：`seller01`
- 固定 completed 订单：`orderId = 900003`
