...
public class EmployeeController {
    ...
    public Result<AuthResponse> login(@Valid @RequestBody PasswordLoginRequest req) {
        ...
        if (!StringUtils.hasText(loginId)) throw new BusinessException("账号不能为空");

        User user = resolveUser(loginId.trim());
        if (user == null) throw new BusinessException("账号不存在");

        // 最小实现：用用户名前缀判断管理员（你库里是 admin / admin01）
    ...
    private User resolveUser(String loginId) {
        if (loginId.contains("@")) return userMapper.selectByEmail(loginId);
