        ...
        <table class="table-base min-w-[1120px]">
          ...
          <tbody class="table-body">
            <tr v-for="order in orders" :key="order.orderId" class="table-row">
              ...
              <td class="table-cell"><p>{{ order.buyerNickname || `用户 #${order.buyerId}` }}</p><p class="mt-1 text-xs text-gray-500">#{{ order.buyerId }} · {{ maskMobile(order.buyerMobile) }}</p></td>
              <td class="table-cell"><p>{{ order.sellerNickname || `用户 #${order.sellerId}` }}</p><p class="mt-1 text-xs text-gray-500">#{{ order.sellerId }} · {{ maskMobile(order.sellerMobile) }}</p></td>
      ...
      <aside class="ml-auto flex h-full w-full max-w-2xl flex-col bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="订单详情">
        ...
        <div class="custom-scrollbar flex-1 overflow-y-auto p-6">
          ...
          <div v-else-if="selectedOrder" class="space-y-6">
            ...
            <section class="grid grid-cols-1 gap-3 md:grid-cols-2"><div class="rounded-lg border border-gray-200 p-4"><h3 class="text-sm font-semibold text-gray-900">买家</h3><p class="mt-2">{{ selectedOrder.buyerNickname }} #{{ selectedOrder.buyerId }}</p><p class="mt-1 text-sm text-gray-500">{{ maskMobile(selectedOrder.buyerMobile) }}</p></div><div class="rounded-lg border border-gray-200 p-4"><h3 class="text-sm font-semibold text-gray-900">卖家</h3><p class="mt-2">{{ selectedOrder.sellerNickname }} #{{ selectedOrder.sellerId }}</p><p class="mt-1 text-sm text-gray-500">{{ maskMobile(selectedOrder.sellerMobile) }}</p></div></section>
