...
const canSubmit = computed(() => Boolean(
  productId.value !== null && product.value && selectedAddress.value?.id && !productLoading.value && !addressLoading.value && !submitting.value,
))
...
async function loadProduct(id: number, sequence: number) {
  ...
  try {
    ...
    if (sequence === requestSequence) product.value = result
  } catch (error: unknown) {
    if (sequence === requestSequence) productError.value = readErrorMessage(error, '商品详情暂时无法加载，请稍后重试。')
  } finally {
    if (sequence === requestSequence) productLoading.value = false
  }
}

async function loadAddresses(sequence: number) {
  ...
  try {
    ...
    if (sequence === requestSequence) addressError.value = readErrorMessage(error, '地址列表暂时无法加载，请稍后重试。')
  } finally {
    if (sequence === requestSequence) addressLoading.value = false
  }
}
...
async function loadCheckout() {
  ...
  if (id === null) return
  await Promise.all([loadProduct(id, sequence), loadAddresses(sequence)])
}
...
async function refreshAddresses() {
  await loadAddresses(requestSequence)
}

async function submitOrder() {
  const id = productId.value
  const addressId = selectedAddressId.value
  if (id === null || addressId === null || submitting.value) return
    ...
    <template v-else>
      ...
      <template v-else>
        <section class="section-panel">
          ...
          <div v-if="productLoading || !product" class="section-body flex min-h-40 items-center justify-center gap-3 text-gray-500"><Loader2 class="h-5 w-5 animate-spin" />正在加载商品</div>
        ...
        <section class="section-panel">
          ...
          <div v-if="addressLoading" class="section-body flex min-h-32 items-center gap-3 text-gray-500"><Loader2 class="h-5 w-5 animate-spin" />正在加载地址</div>
          <div v-else-if="addressError" class="section-body"><div class="notice-banner notice-banner-danger"><span>{{ addressError }}</span></div><button class="btn-default mt-4" type="button" :disabled="submitting" @click="refreshAddresses">重新加载地址</button></div>
        ...
        <section v-if="submitError" class="notice-banner notice-banner-danger"><div><p>{{ submitError }}</p><router-link v-if="shouldReturnToMarket" class="btn-default mt-3" to="/market">返回市场</router-link></div></section>
        <section v-if="hasAddress" class="flex flex-wrap items-center justify-between gap-4 border-t border-gray-200 pt-5"><p v-if="product" class="font-semibold text-gray-900">订单总额 <span class="font-numeric text-[20px]">¥ {{ product.price.toFixed(2) }}</span></p><button class="btn-primary" type="button" :disabled="!canSubmit" @click="submitOrder"><Loader2 v-if="submitting" class="h-4 w-4 animate-spin" /><span>{{ submitting ? '正在创建订单' : '确认订单' }}</span></button></section>
      </template>
    </template>
  </main>
</template>
