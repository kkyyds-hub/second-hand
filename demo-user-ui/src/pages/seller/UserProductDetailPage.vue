...
const statusDescription = computed(() => {
  const descriptions: Record<string, string> = {
    under_review: '商品正在审核，审核完成前不会在市场展示。',
    on_sale: '商品正在市场和公开小店中展示。',
    off_shelf: '商品当前已下架，不会在市场展示。',
    sold: '商品已售出，不能继续编辑。',
  }
  return descriptions[detail.value.status] || '商品当前状态以页面展示为准。'
})
...
function isRunningStatusAction(meta: UserProductStatusActionMeta) {
  return runningAction.value && pendingAction.value?.kind === 'status' && pendingAction.value.meta.action === meta.action
}
function isDeletingProduct() {
  return runningAction.value && pendingAction.value?.kind === 'delete'
}
...
async function loadDetail(options?: { preserveDetail?: boolean; suppressError?: boolean }) {
  ...
  loadError.value = ''
  if (!id) { loading.value = false; hasLoadedOnce.value = true; loadError.value = '商品信息无效，请从商品管理重新进入。'; return false }
...
function reloadDetail() { void loadDetail() }
...
<template>
  <main class="page-body">
    ...
    <template v-else>
      ...
      <template v-else>
        <section class="section-panel"><div class="section-body"><div class="grid gap-8 lg:grid-cols-[minmax(0,420px)_minmax(0,1fr)]"><ProductImageGallery :product-title="detail.title" :image-urls="detail.imageUrls" /><div class="min-w-0 space-y-5"><div class="space-y-3 border-b border-gray-100 pb-5"><div class="flex flex-wrap items-center gap-2"><span :class="statusChipClass()">{{ statusMeta.label }}</span><span v-if="detail.category" class="chip chip-neutral break-all">{{ detail.category }}</span></div><h2 class="break-words text-[26px] font-bold text-gray-900">{{ detail.title || '未命名商品' }}</h2><p class="font-numeric text-[30px] font-bold text-gray-900">¥{{ detail.price.toFixed(2) }}</p><p class="text-[13px] leading-6 text-gray-600">{{ statusDescription }}</p></div><div class="flex flex-wrap gap-2"><router-link v-if="detail.status === 'on_sale' && productId" class="btn-default !h-9 px-3" :to="`/market/${productId}`">查看市场展示</router-link><button v-for="meta in statusActions" :key="meta.action" :class="actionButtonClass(meta)" type="button" :disabled="runningAction" @click="openStatusAction(meta)"><Loader2 v-if="isRunningStatusAction(meta)" class="h-4 w-4 animate-spin" aria-hidden="true" /><span>{{ meta.label }}</span></button></div><p v-if="detail.status === 'on_sale' && !statusActions.length" class="text-[13px] text-gray-500">在售商品需先下架后才能删除</p><div class="detail-grid"><div class="detail-row"><span class="detail-label">创建时间</span><span class="detail-value font-numeric">{{ detail.createTime || '-' }}</span></div><div class="detail-row"><span class="detail-label">更新时间</span><span class="detail-value font-numeric">{{ detail.updateTime || '-' }}</span></div><div class="detail-row"><span class="detail-label">最近提交审核时间</span><span class="detail-value font-numeric">{{ detail.submitTime || '-' }}</span></div></div></div></div></div></section>
        <section v-if="detail.reviewRemark || detail.reason" class="section-panel-muted"><div class="section-body"><h2 class="section-heading">审核与状态说明</h2><p class="mt-2 whitespace-pre-wrap break-words text-[13px] leading-6 text-orange-800">{{ detail.reviewRemark || detail.reason }}</p></div></section>
