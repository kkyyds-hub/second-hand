...
const submitAvatar = async () => {
  ...
  if (!hasSessionProfile.value) {
    uploadStatus.value = 'error'
    uploadMessage.value = '账户信息暂时无法读取，请重新登录后再试。'
    return
  }
...
<template>
  <div class="page-body page-body-narrow">
    <section class="page-header">
      <div class="page-header-main"><p class="page-kicker">头像</p><h1 class="page-title">头像设置</h1><p class="page-desc">选择 JPG 或 PNG 图片后，可先确认预览再上传。</p></div>
      ...
      <div class="section-panel-muted">
        ...
        <div class="section-body pt-0 space-y-4">
          <div class="flex flex-col items-center rounded-2xl border border-gray-200/70 bg-white px-6 py-8 text-center">
            <img v-if="previewAvatarUrl && !previewImageFailed" :src="previewAvatarUrl" :alt="`${displayName}的头像预览`" class="h-28 w-28 rounded-full border border-gray-200 object-cover shadow-sm bg-white" @error="previewImageFailed = true" />
