...
3. 当前环境 RabbitMQ 发布链路异常，恢复到 `SENT` 未达成（已记录）。
...
1. P7-S2 目标已达成：并发分流稳定、失败恢复路径可复现。
2. 遗留：需在 RabbitMQ 可用环境补一次 Outbox `FAIL -> SENT` 恢复实证。
3. 遗留：补跑 `com.demo.concurrency` 单测包，完成编译态闭环。
